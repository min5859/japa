...
### [~] Transaction (거래 내역) 추적 — **MVP 완료 (2026-05-09)**, Tax 통합·CSV는 후속
- **MVP 완료분**:
  - `Transaction` 모델 (BUY/SELL only) + 마이그레이션 `20260508232227_add_transaction`
  - `app/actions/transactions.ts`: 매수 시 평단가 가중평균(수수료 취득원가 포함), 매도 시 평단 유지·실현손익 row 보존, 옵션 ON 시 `Account.cashBalance` 자동 갱신, 삭제 시 효과 역연산 (모두 `prisma.$transaction`)
  - `/holdings/[id]` 신설 detail 페이지 — 매수/매도 진입 + 거래 히스토리 표 + 누적 실현손익 카드
  - `/holdings/[id]/trade/new?type=BUY|SELL` 거래 입력 페이지 + `TransactionForm`
  - 진입점: `/holdings`·계좌 detail의 자산명 → detail 링크, 계좌 detail에 매수/매도 단축 아이콘 추가
- **남은 후속 작업** (별도 세션):
  - Tax 양도세 페이지 통합: `lib/tax.ts:calcForeignGains`를 SELL transaction 기반 실현 손익과 결합 → "실현 + 미실현 분리"
  - `/api/export/transactions` CSV export
  - DEPOSIT/WITHDRAW/FEE/REINVEST 등 enum 확장 (필요 시)
  - 종목 통화와 거래 통화 다른 경우 자동 cashBalance 갱신 (현재는 통화 일치 시에만 적용)
- **원래 한계 (참고)**:
